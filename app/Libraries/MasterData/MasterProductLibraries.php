...
class MasterProductLibraries
{
	...
	public function import_file($request)
	{
		...
		// $find = MasterBOM::where('IsDelete', 0)->update([
		// 	'User_Updated'	=> $user_updated,
		// 	'IsDelete'		=> 1
		// ]);
		$arr = [];
		...
		if ($fileName == 'xlsx' || $fileName == 'xls') {
			foreach ($data as $key => $value) {
				if ($key > 2) {
					if ($value[2] != '' && strlen($value[2]) <= 20 && $value[1] != '' && strlen($value[1]) <= 20 && is_numeric($value[5]) && is_numeric($value[6]) && is_numeric($value[7]) && $value[5] > 0  && $value[6] > 0 && $value[7] > 0 && $value[3] != '' && strlen($value[3]) <= 20) {
						$find_mold = MasterMold::where('IsDelete', 0)->where('Symbols', $value[1])->first();
						$find_product = MasterProduct::where('IsDelete', 0)->where('Symbols', $value[2])->first();
						$find_materials = MasterMaterials::where('IsDelete', 0)->where('Name', $value[4])->where('Symbols', $value[3])->first();
						if ($find_product) {
						} else {
						...
						array_push($arr[$find_product->ID], $find_materials->ID);
						$find = MasterBOM::where('IsDelete', 0)->where('Product_BOM_ID', $find_product->ID)->where('Materials_ID', $find_materials->ID)->first();
						if (!$find) {
							MasterBOM::create([
							...
							if ($find->Quantity_Material != $value[6]) {
								MasterBOM::where('ID', $find->ID)->update([
									'Quantity_Material' => $value[6],
									'User_Updated'	=> $user_updated,
								]);
							}
						...
						array_push($arr1[$find_product->ID], $find_mold->ID);
						$find_mold1 = MasterBOM::where('IsDelete', 0)->where('Product_BOM_ID', $find_product->ID)->where('Mold_ID', $find_mold->ID)->first();
						if (!$find_mold1) {
							MasterBOM::create([
								'Product_BOM_ID' 	=> $find_product->ID,
							...
							if ($find_mold1->Cavity != $value[7] || $find_mold1->Cycle_Time != $value[5]) {
								MasterBOM::where('ID', $find_mold1->ID)->update([
									'Cavity' 			=> $value[7],
									'Cavity_Real' 		=> $value[7],
									'Cycle_Time'		=> $value[5],
									'User_Updated'		=> $user_updated,
			...
			foreach ($arr as $key => $value) {
				MasterBOM::where('IsDelete', 0)->where('Product_BOM_ID', $key)->where('Materials_ID', '>', 0)->whereNotIn('Materials_ID', $value)->update([
					'User_Updated'	=> $user_updated,
					'IsDelete'		=> 1
				]);
			}
			foreach ($arr1 as $key => $value) {
				MasterBOM::where('IsDelete', 0)->where('Product_BOM_ID', $key)->where('Mold_ID', '>', 0)->whereNotIn('Mold_ID', $value)->update([
					'User_Updated'	=> $user_updated,
					'IsDelete'		=> 1
				]);
			}
